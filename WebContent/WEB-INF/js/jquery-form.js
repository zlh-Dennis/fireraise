...
(function(factory) {
	if (typeof define === 'function' && define.amd) {
		...
		(function($) {
			 ...
			 * form. For example, $(document).ready(function() {
			 * $('#myForm').on('submit', function(e) { e.preventDefault(); //
			 * <-- important $(this).ajaxSubmit({ target: '#output' }); }); });
			 * Use ajaxForm when you want the plugin to manage all the event
			 * binding for you. For example, $(document).ready(function() {
			 * $('#myForm').ajaxForm({ target: '#output' }); }); You can also
			 * use ajaxForm with delegation (requires jQuery v1.7+), so the form
			 * does not have to exist when you invoke ajaxForm:
			 ...
			 * 
			 * @param {object|string}
			 *            options jquery.form.js parameters or custom url for
			 *            submission
			 * @param {object}
			 *            data extraData
			 * @param {string}
			 ...
			 * @param {function}
			 *            onSuccess ajax success callback function
			 */
			$.fn.ajaxSubmit = function(options, data, dataType, onSuccess) {
				...
				options = $.extend(true,
						{
							...
							iframeSrc : /^https/i.test(window.location.href
									|| '') ? 'javascript:false' : 'about:blank' // eslint-disable-line
																				// no-script-url
						}, options);
				...
				if (options.data) {
					var optionsData = $.isFunction(options.data) ? options
							.data(a) : options.data;
				...
				if (!options.dataType && options.target) {
					...
					callbacks
							.push(function(data, textStatus, jqXHR) {
								var successArguments = arguments, fn = options.replaceTarget ? 'replaceWith'
										: 'html';
				...
				function fileUploadIframe(a) {
					...
					function doSubmit() {
						...
						function checkState() {
							try {
								...
								cb(SERVER_ABORT); // eslint-disable-line
													// callback-return
								if (timeoutHandle) {
						...
						try {
							...
							if (scr || s.textarea) {
								...
								if (ta) {
									...
									&& xhr.responseText) {
								xhr.responseXML = toXml(xhr.responseText); // eslint-disable-line
																			// no-use-before-define
							}

							try {
								data = httpData(xhr, dt, s); // eslint-disable-line
																// no-use-before-define
					...
					var parseJSON = $.parseJSON || function(s) {
						/* jslint evil:true */
						return window['eval']('(' + s + ')'); // eslint-disable-line
																// dot-notation
					};
					...
					var httpData = function(xhr, type, s) { // mostly lifted
						...
						var ct = xhr.getResponseHeader('content-type') || '', xml = ((type === 'xml' || !type) && ct
								.indexOf('xml') >= 0), data = xml ? xhr.responseXML
								: xhr.responseText;
			...
			$.fn.formToArray = function(semantic, elements, filtering) {
				...
				// 'form' attribute
				// FinesseRus: in non-IE browsers outside fields are already
				// included in form.elements.
				if (formId
						&& (semantic || /(Edge|Trident)\//
			 ...
			 * actions on the selected tags: - all fields within form elements
			 * will be reset to their original value - input / textarea / select
			 * fields will be reset to their original value - option / optgroup
			 * fields (for multi-selects) will defaulted individually -
			 * non-multiple options will find the right select to default -
			 * label elements will be searched against its 'for' attribute - all
			 * others will be searched for appropriate children to default
			 */
			$.fn.resetForm = function() {
				return this
						.each(function() {
							...
							case 'form':
								// guard against an input with the name of
								// 'reset'
							...
							default:
								el.find('form,input,label,select,textarea')
										.resetForm();
